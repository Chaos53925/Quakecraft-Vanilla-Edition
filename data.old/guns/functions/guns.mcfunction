...
# Waffen
execute as @a[tag=canShoot] at @s if score @s rightclick matches 1.. run function guns:sounds
execute as @a[tag=!canShoot] at @s if score @s rightclick matches 1.. positioned as @s run playsound minecraft:block.dispenser.fail master @s[team=!Lobby,distance=..50,tag=!soundsoff] ~ ~ ~ 0.4 1 0

# Auslösen des Schusses
execute as @a[tag=canShoot,scores={ammo=1..,rightclick=1..}] run function guns:weapons/weaponselector
