#Partikeleffekt
execute if score @s particleC matches 0 run function custom:guns/core/particle
...
#Hitbox erkennung
execute as @e[dx=0,tag=!die] positioned ~-0.85 ~-0.85 ~-0.85 if entity @s[dx=0] positioned ~0.85 ~0.85 ~0.85 run function custom:guns/core/hitbox
execute at @e[tag=hit,tag=!immune] run function custom:guns/railgun/beam
...
execute if score @s railgun <= MAX railgun positioned ^ ^ ^0.25 if block ~ ~ ~ #custom:bangable run function custom:guns/railgun/core

#reset distanz
execute if score @s railgun <= MAX railgun positioned ^ ^ ^0.25 unless block ~ ~ ~ #custom:bangable run scoreboard players reset @s distance
